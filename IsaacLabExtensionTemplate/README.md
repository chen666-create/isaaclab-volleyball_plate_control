...
- Throughout the repository, the name `Plate_robot` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of Plate_robot (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/Plate_robot
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/Plate_robot/Plate_robot/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/Plate_robot"
    ]
}
```
